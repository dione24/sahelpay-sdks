...
const sahelpay = new SahelPay({
  ...
  environment: 'sandbox' // sk_test_... utilise https://api.sahelpay.ml
});
```
...
const payment = await sahelpay.payments.create({
  ...
  sandbox: true, // simulateur SahelPay en test
});
...
await sahelpay.webhooks.test(); // envoie webhook.test vers votre endpoint configuré
```
...
```typescript
const result = await sahelpay.payments.checkStatus('txn_abc123');

console.log(result.status); // 'PENDING' | 'SUCCESS' | 'FAILED' | 'EXPIRED'
console.log(result.payment.amount);
console.log(result.payment.provider_ref);
```
...
export async function POST(request: Request) {
  ...
  let payload;
  try {
    payload = sahelpay.webhooks.constructEvent(
      rawBody,
      signature,
