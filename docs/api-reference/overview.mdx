...
Le sandbox est déterminé par la clé `sk_test_...`. Pour le simulateur SahelPay sans appel provider, ajoutez `metadata.sandbox: true` lors de la création du paiement.
...
| `/v1/webhooks/test` | `POST` | Envoyer un webhook `webhook.test` vers l'URL configurée |
