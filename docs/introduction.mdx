...
|---------------|-----|------|
| **Sandbox** | `https://api.sahelpay.ml` | `sk_test_xxx` |
...
<Note>
  Utilisez toujours une clé `sk_test_...` pour vos tests. Pour simuler un paiement sans appel provider, envoyez `metadata.sandbox: true` ou l'option SDK `sandbox: true`.
</Note>
