...
<Note>
  En test, `sandbox: true` utilise le simulateur SahelPay et évite tout appel provider réel. Pour tester la sandbox Orange/Wave/Moov, retirez ce flag et gardez une clé `sk_test_...`.
</Note>
...
const result = await sahelpay.payments.checkStatus(paymentId);
