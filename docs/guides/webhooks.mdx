...
|-----------|-------------|
| `webhook.test` | Test de connectivité envoyé par `/v1/webhooks/test` |
...
Pour tester la connectivité sans créer de paiement, utilisez l'endpoint de test ou le helper SDK:
...
```php PHP
$sahelpay->webhooks->test();
```

</CodeGroup>

Cet appel envoie `webhook.test` vers l'URL configurée dans le dashboard. Pour tester un événement métier `payment.success` ou `payment.failed`, créez un paiement avec `metadata.sandbox: true`, puis appelez `GET /v1/payments/{id}/status`.
